...
class OfficeProfileCollection extends BaseProfileCollection {
  ...
  /**
   * Updates the OfficeProfile. You cannot change the email or role.
   * @param docID the id of the OfficeProfile
  ...
  checkIntegrity() {
    ...
    this.find().forEach((doc) => {
      if (doc.role !== ROLE.User) {
        problems.push(`OfficeProfile instance does not have ROLE.Office: ${doc}`);
      }
    });
    return problems;
  }
  ...
  /**
   * Returns an object representing the OfficeProfile docID in a format acceptable to define().
   * @param docID The docID of a OfficeProfile
