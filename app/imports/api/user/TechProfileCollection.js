...
class TechProfileCollection extends BaseProfileCollection {
  constructor() {
    super('TechProfile', new SimpleSchema({}));
  }
  ...
  /**
   * Updates the TechProfile. You cannot change the email or role.
   * @param docID the id of the TechProfile
  ...
  checkIntegrity() {
    ...
    this.find().forEach((doc) => {
      if (doc.role !== ROLE.User) {
        problems.push(`TechProfile instance does not have ROLE.Tech: ${doc}`);
      }
    });
    return problems;
  }
  ...
  /**
   * Returns an object representing the TechProfile docID in a format acceptable to define().
   * @param docID The docID of a TechProfile
 ...
 * @type {TechProfileCollection}
 */
export const TechProfiles = new TechProfileCollection();
